...
class SIS:
    ...
    def _initiate_network(self) -> ig.Graph:
        ...
        ## Sus
        susceptible_nodes = [
            x for x in g.vs if x.index not in infected_nodes_idx
        ]  ## getting the susceptible nodes
        susceptible_nodes_idx = [
            x.index for x in susceptible_nodes
        ]  ## getting their index
    ...
    def _run_sis(self):
        ...
        ## -- -- -- -- -- -- -- -- -- -- -- -- -- ##
        inf = np.array(
            [x for x in self.graph.vs if x["status"] == "I"]
        )  ## Infected Nodes
